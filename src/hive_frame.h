...
typedef enum {
	...
	RECV_SERVER_PREFACE =
	    1, /* client: first frame must be SETTINGS non-ACK */
	RECV_FRAME_HEADER =
	    2, /* accumulate frame_hdr_buf[9] via frame_hdr_count */
	RECV_DATA_PAYLOAD =
	    3,             /* streaming DATA delivery into caller's buffer */
	RECV_DATA_PAD = 4, /* skip padding bytes after DATA payload */
	RECV_HEADERS_PAYLOAD =
	    5,                /* copy header block bytes into reassembly_buf */
	RECV_HEADERS_PAD = 6, /* skip padding bytes after HEADERS payload */
	RECV_CONTINUATION_PAYLOAD =
	    7, /* continue header block; any other frame = conn error */
	RECV_PUSH_PROMISE_PAYLOAD =
	    8, /* extract 4-byte promised stream_id, copy remainder */
	RECV_PUSH_PROMISE_PAD =
	    9, /* skip padding bytes after PUSH_PROMISE payload */
	RECV_SETTINGS_PAYLOAD =
	    10,                 /* process 6-byte parameters via ctrl_staging */
	...
	RECV_WINDOW_UPDATE_PAYLOAD =
	    13, /* accumulate 4 bytes via ctrl_staging */
	RECV_GOAWAY_PAYLOAD =
	    14, /* accumulate first 8 bytes (last_stream_id + error_code) */
	RECV_GOAWAY_DEBUG =
...
void frame_recv_init(hive_session_t *s, hive_role_t role);
ssize_t frame_recv_process(hive_session_t *s, const uint8_t *data, size_t len);
