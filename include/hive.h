...
/*
 * Header name/value pair — used for submit functions.
 */
...
typedef ssize_t (*hive_read_callback_t)(hive_session_t *session,
    uint32_t stream_id, uint8_t **buf, uint32_t flags, void *user_data);
...
#define HIVE_DATA_FLAG_NO_COPY	0x02u	/* zero-copy: redirect *buf to caller mem */
...
typedef struct hive_callbacks {
	...
	int (*on_headers_complete)(hive_session_t *session,
	    uint32_t stream_id, uint8_t flags, void *user_data);
	...
	int (*on_ping)(hive_session_t *session,
	    const uint8_t opaque[8], void *user_data);
...
int	hive_options_set_max_concurrent_streams(hive_options_t *opt,
	    uint32_t v);
...
int	hive_options_set_rst_flood_window_secs(hive_options_t *opt,
	    uint32_t v);
