...
{
	static const uint8_t block[] = {
		/* RFC 7541 C.6.1: first response with Huffman coding */
		0x48, 0x82, 0x64, 0x02,
	...
	ASSERT(ret == HIVE_OK);
	ASSERT(cap.header_count == 4);
	ASSERT(strcmp(cap.names[0], ":status") == 0);
	ASSERT(strcmp(cap.values[0], "302") == 0);
	ASSERT(strcmp(cap.names[1], "cache-control") == 0);
	ASSERT(strcmp(cap.values[1], "private") == 0);
	ASSERT(strcmp(cap.names[2], "date") == 0);
	ASSERT(strcmp(cap.values[2], "Mon, 21 Oct 2013 20:13:21 GMT") == 0);
	ASSERT(strcmp(cap.names[3], "location") == 0);
	ASSERT(strcmp(cap.values[3], "https://www.example.com") == 0);
